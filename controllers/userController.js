...
exports.getstockdata = async (req, res, next) => {
  ...
  try {
  if(req.query.symbol!=''){
    var query1 = "SELECT company_code.company_name, company_code.symbol,symbol_date.date,symbol_data.open,symbol_data.close,symbol_data.high,symbol_data.low,symbol_data.volume,symbol_data.adjclose FROM `company_code` JOIN `symbol_date` ON company_code.id = symbol_date.ccid JOIN `symbol_data` ON symbol_date.id = symbol_data.symbol_dt_id WHERE company_code.symbol = ?";  
  ...
  if(req.query.year!=''){
    var query1 = "SELECT company_code.company_name, company_code.symbol,symbol_date.date,symbol_data.open,symbol_data.close,symbol_data.high,symbol_data.low,symbol_data.volume,symbol_data.adjclose FROM company_code JOIN symbol_date ON company_code.id = symbol_date.ccid JOIN symbol_data ON symbol_date.id = symbol_data.symbol_dt_id WHERE YEAR(symbol_date.date) = ?";  
    var symbol= req.query.year;
    let row = await query(
      query1,
      [symbol]
     );
  ...
  if(req.query.month!=''){
    var query1 = "SELECT company_code.company_name, company_code.symbol,symbol_date.date,symbol_data.open,symbol_data.close,symbol_data.high,symbol_data.low,symbol_data.volume,symbol_data.adjclose FROM company_code JOIN symbol_date ON company_code.id = symbol_date.ccid JOIN symbol_data ON symbol_date.id = symbol_data.symbol_dt_id WHERE MONTHNAME(symbol_date.date) = ?";
    var symbol = req.query.month;
  ...
  if(req.params['month']!='' && req.params['year']!='' ){
    ...
    var year  = req.params['year'];
     ...
     let row = await query(
       "SELECT company_code.company_name, company_code.symbol,symbol_date.date,MONTHNAME(symbol_date.date),symbol_data.open,symbol_data.close,symbol_data.high,symbol_data.low,symbol_data.volume,symbol_data.adjclose FROM company_code JOIN symbol_date ON company_code.id = symbol_date.ccid JOIN symbol_data ON symbol_date.id = symbol_data.symbol_dt_id WHERE YEAR(symbol_date.date) = ? AND MONTHNAME(symbol_date.date) = ?",
       [year,month]
       );
   ...
   } // Year and Month Condition Combined

   if(req.params['symbol'] && req.params['month']!='' && req.params['year']!=''){
    var month = req.params['month'];
    var year  = req.params['year'];
    var symbol = req.params['symbol'];
    
     let row = await query(
       "SELECT company_code.company_name, company_code.symbol,symbol_date.date,symbol_data.open,symbol_data.close,symbol_data.high,symbol_data.low,symbol_data.volume,symbol_data.adjclose FROM company_code JOIN symbol_date ON company_code.id = symbol_date.ccid JOIN symbol_data ON symbol_date.id = symbol_data.symbol_dt_id WHERE company_code.symbol = ? AND MONTHNAME(symbol_date.date) = ? AND YEAR(symbol_date.date) = ?",
       [symbol,month,year]
       );
   ...
   } // Symbol , Month and Year Condition Combined
